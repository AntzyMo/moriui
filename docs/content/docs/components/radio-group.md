...
description: 用于单项选择场景的 MoriUI 组件。
---
...
RadioGroup 管理唯一选中值，RadioGroupItem 以必填 value 标识选项；标签、说明与错误由 Field 组合。

### 受控与默认值

使用 v-model 持有当前 AcceptableValue；无需受控时可用 defaultValue 设置初始选择。

### 禁用与无效

disabled 可设置在整组或单个 Item；无效项使用 aria-invalid，外围 Field 使用 data-invalid。

### RTL

dir 影响方向键顺序与内容阅读方向，Reka UI 负责 roving focus 和循环导航。
