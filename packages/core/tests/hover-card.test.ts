      ...
      default: () => [
        ...
        h(HoverCardPortal, {}, {
          default: () => h(HoverCardContent, {
            ...
            default: () => [
              ...(options.showArrow ? [h(HoverCardArrow, { class: 'custom-arrow' })] : []),
    ...
    setup: () => () => h(HoverCard, {
      ...
      default: () => [
        h(HoverCardTrigger, {
          ...
          default: () => h('button', { 'data-testid': 'default-delay-trigger' }, '查看个人资料')
        }),
        h(HoverCardPortal, {}, {
          default: () => h(HoverCardContent, {}, {
            default: () => 'MoriUI 组件库'
...
it('悬停后打开、显式 Portal 到 body，并保留稳定槽位与调用方类名', async () => {
  ...
  expect(content.querySelector('[data-slot="hover-card-arrow"]')?.classList).toContain('hover-card__arrow')
  expect(content.querySelector('[data-slot="hover-card-arrow"]')?.classList).toContain('custom-arrow')
...
it('内容消费 Nova 浮层 Token、固定宽度与方向动效', async () => {
  ...
  expect(getComputedStyle(content).getPropertyValue('--hover-card-content-bg').trim()).toBe('oklch(1 0 0)')
  expect(getComputedStyle(content).getPropertyValue('--hover-card-slide').trim()).toBe('0 -0.5rem')

  document.documentElement.dataset.theme = 'dark'
  expect(getComputedStyle(content).getPropertyValue('--hover-card-content-bg').trim()).toBe('oklch(0.205 0 0)')
})
