...
function getDateTrigger(container: HTMLElement, date: string) {
  return container.querySelector(
    `[data-slot="calendar-cell-trigger"][data-value="${date}"]`
  ) as HTMLElement
}
...
it('渲染稳定槽位、默认导航图标与紧凑网格样式', () => {
  ...
  expect(getComputedStyle(calendar).getPropertyValue('--background').trim()).toBe(
    'oklch(0.145 0 0)'
  )
})
...
it('遵守日期边界、不可用日期、只读与整组件禁用状态', async () => {
  ...
  const constrainedCalendar = constrained.container.querySelector(
    '[data-slot="calendar"]'
  ) as HTMLElement

  expect(
    (constrainedCalendar.querySelector('[data-slot="calendar-prev"]') as HTMLButtonElement)
      .disabled
  ).toBe(true)
  expect(getDateTrigger(constrainedCalendar, '2024-04-09').hasAttribute('data-disabled')).toBe(
    true
  )
  expect(getDateTrigger(constrainedCalendar, '2024-04-15').hasAttribute('data-unavailable')).toBe(
    true
  )
  ...
  const readonlyCalendar = readonly.container.querySelector(
    '[data-slot="calendar"]'
  ) as HTMLElement
  ...
  const disabledCalendar = disabled.container.querySelector(
    '[data-slot="calendar"]'
  ) as HTMLElement
  expect(
    (disabledCalendar.querySelector('[data-slot="calendar-next"]') as HTMLButtonElement).disabled
  ).toBe(true)
  expect(
    disabledCalendar.querySelector('[data-slot="calendar-grid"]')?.hasAttribute('data-disabled')
  ).toBe(true)
})
...
it('完整透传标题与日期作用域插槽', () => {
  const Fixture = defineComponent({
    setup: () => () =>
      h(
        Calendar,
        { defaultPlaceholder: april, locale: 'en-US' },
        {
          heading: ({ headingValue }) =>
            h('span', { 'data-testid': 'heading' }, `当前：${headingValue}`),
          day: ({ dayValue, today }) =>
            h('span', { 'data-testid': `day-${dayValue}` }, today ? `今日 ${dayValue}` : dayValue)
        }
      )
  })
  const page = render(Fixture)
