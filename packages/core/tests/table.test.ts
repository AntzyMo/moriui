...
it('渲染 TanStack 列定义、稳定槽位、横向滚动与默认分页', () => {
  ...
  const scrollContainer = page.container.querySelector(
  ...
  expect(
    page.container.querySelectorAll('[data-slot="table-body"] [data-slot="table-row"]')
  ).toHaveLength(10)
  ...
  expect(
    page.container.querySelector('[data-slot="table-pagination-summary"]')?.textContent
  ).toContain('12')
})
...
it('支持排序、全局搜索与列级文本筛选，并同步对应模型事件', async () => {
  ...
  const sorter = page.container.querySelector(
...
it('支持当前页多选、分页切换和页大小切换', async () => {
  ...
  const pageSize = page.container.querySelector(
...
it('支持受控状态、工具栏和分页插槽替换', async () => {
  ...
  expect(page.container.querySelector('[data-testid="custom-toolbar"]')?.textContent).toBe(
    '自定义工具栏'
  )
  expect(page.container.querySelector('[data-testid="custom-pagination"]')?.textContent).toBe(
    '自定义分页'
  )
...
it('在空数据时渲染默认或自定义空状态', () => {
  ...
  expect(defaultPage.container.querySelector('[data-slot="table-empty"]')?.textContent).toContain(
    '没有结果'
  )
  expect(customPage.container.querySelector('[data-testid="custom-empty"]')?.textContent).toBe(
    '自定义空状态'
  )
})
