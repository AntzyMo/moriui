                ...
                {},
                {
                  default: () => [
                    ...
                    h(
                      ...
                      {
                        default: () =>
                          h(BubbleContent, {}, { default: () => '你好，我可以协助处理这个问题。' })
                      }
                    ),
          ...
          {},
          {
            default: () => [
              ...
              h(
                Bubble,
                { variant: 'ghost' },
                { default: () => h(BubbleContent, {}, { default: () => '长文本回复' }) }
              ),
...
it('六个组件均支持多态渲染', () => {
  ...
  expect(render(ContentFixture).container.querySelector('section')?.dataset.slot).toBe(
    'message-content'
  )
  expect(render(FooterFixture).container.querySelector('footer')?.dataset.slot).toBe(
    'message-footer'
  )
  expect(render(GroupFixture).container.querySelector('ol')?.dataset.slot).toBe('message-group')
  expect(render(HeaderFixture).container.querySelector('header')?.dataset.slot).toBe(
    'message-header'
  )
})
...
it('头像与元信息使用当前主题的语义 Token', () => {
  ...
  const lightMutedForeground = getComputedStyle(header)
  ...
  expect(getComputedStyle(header).getPropertyValue('--muted-foreground').trim()).not.toBe(
