  ...
  const Fixture = defineComponent({
    setup: () => () => h(Drawer, {
      ...
      default: () => [
        ...
        h(DrawerContent, { class: 'custom-content', showHandle: options.showHandle }, {
          default: () => [
            h(DrawerHeader, { class: 'custom-header' }, {
              default: () => [
                h(DrawerTitle, {}, { default: () => '编辑项目' }),
                h(DrawerDescription, {}, { default: () => '更新项目的基本信息。' })
              ]
            }),
            h(DrawerFooter, { class: 'custom-footer' }, {
              default: () => [
...
it.each<SwipeDirection>(['down', 'up', 'left', 'right'])('渲染 %s 方向、稳定槽位和调用方类名', async swipeDirection => {
  const { Fixture } = createFixture({ showHandle: true, swipeDirection })
  const { trigger } = await openDrawer(Fixture)
  const content = document.body.querySelector('[data-slot="drawer-content"][data-state="open"]') as HTMLElement
  ...
  expect(document.body.querySelector('[data-slot="drawer-overlay"]')?.classList).toContain('drawer__overlay')
  expect(content.querySelector('[data-slot="drawer-handle"]')?.classList).toContain('drawer__handle')
  expect(content.querySelector('[data-slot="drawer-header"]')?.classList).toContain('custom-header')
  expect(content.querySelector('[data-slot="drawer-footer"]')?.classList).toContain('custom-footer')
})
...
it('右侧 Drawer 使用 Nova 浮动面板与固定操作区', async () => {
  ...
  const overlay = document.body.querySelector('[data-slot="drawer-overlay"][data-state="open"]') as HTMLElement
...
it('通过受控 open 模型同步开闭状态', async () => {
  ...
  const close = document.body.querySelector('[data-slot="drawer-content"] [data-slot="drawer-close"]') as HTMLButtonElement
  close.click()
  await nextTick()
