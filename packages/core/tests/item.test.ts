...
it('渲染完整组合槽位、媒体变体、分组与水平分隔线', () => {
              ...
              { size: 'xs' },
              {
                default: () => [
                  h(
                    ItemMedia,
                    { variant: 'image' },
                    { default: () => h('img', { alt: '项目预览', src: '/project.png' }) }
                  ),
                  h(
                    ItemContent,
                    {},
                    {
                      default: () => [
                        h(
                          ItemHeader,
                          {},
                          { default: () => h(ItemTitle, {}, { default: () => '项目概览' }) }
                        ),
                        h(
                          ItemDescription,
                          {},
                          { default: () => h('a', { href: '#details' }, '查看项目详情。') }
                        ),
  ...
  expect(page.container.querySelector('[data-slot="item-content"]')?.classList).toContain(
    'item__content'
  )
  expect(page.container.querySelector('[data-slot="item-title"]')?.classList).toContain(
    'item__title'
  )
  expect(page.container.querySelector('[data-slot="item-description"]')?.classList).toContain(
    'item__description'
  )
  expect(page.container.querySelector('[data-slot="item-actions"]')?.classList).toContain(
    'item__actions'
  )
  expect(page.container.querySelector('[data-slot="item-header"]')?.classList).toContain(
    'item__header'
  )
  expect(page.container.querySelector('[data-slot="item-footer"]')?.classList).toContain(
    'item__footer'
  )
...
it.each(['default', 'icon', 'image'] as const)('应用 %s 媒体变体', variant => {
  ...
  if (variant === 'image') {
    ...
    expect(getComputedStyle(media.querySelector('img') as HTMLImageElement).objectFit).toBe(
      'cover'
    )
  }
})
...
it('消费局部 Token，支持描述媒体布局、焦点环、链接样式和主题切换', () => {
  ...
  expect(getComputedStyle(item).getPropertyValue('--item-hover-bg').trim()).toBe(
    'oklch(0.269 0 0)'
  )
})
