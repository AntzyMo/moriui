...
it('渲染稳定槽位、默认文案、分组标题与调用方类名', () => {
  ...
  expect(page.container.querySelector('[data-slot="command-group-heading"]')?.textContent).toBe(
    '导航'
  )
  expect(page.container.querySelector('[data-slot="command-shortcut"]')?.textContent).toBe('G H')
  expect(page.container.querySelector('[data-slot="command-empty"]')?.textContent).toContain(
    '没有匹配的命令'
  )
})
...
it('筛选、空态和 ignoreFilter 外部筛选模式符合 Reka 契约', async () => {
  ...
  const manualInput = manualPage.container.querySelector(
...
it('禁用项目不可选，并且高亮、明暗主题样式遵循组件 Token', async () => {
  ...
  const items = Array.from(
    page.container.querySelectorAll('[data-slot="command-item"]')
  ) as HTMLElement[]
...
it('commandDialog Teleport、可访问文案和关闭行为均由 Reka Dialog 管理', async () => {
                ...
                {},
                {
                  default: () => [
                    ...
                    h(
                      ...
                      {
                        default: () =>
                          h(CommandItem, { value: 'open' }, { default: () => '打开文件' })
                      }
                    )
                  ]
                }
  ...
  expect(content.querySelector('[data-slot="command-dialog-description"]')?.textContent).toBe(
    '搜索并执行命令。'
  )
