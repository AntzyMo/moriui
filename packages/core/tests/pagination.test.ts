...
function createPagination(options: PaginationOptions = {}) {
  return defineComponent({
    setup: () => () => h(Pagination, {
      ...
      default: () => h(PaginationList, {}, {
        default: ({ items }) => [
          h(PaginationFirst),
          h(PaginationPrev),
          ...items.map((item, index) => item.type === 'page'
            ? h(PaginationItem, { key: `page-${item.value}`, value: item.value }, {
...
function getPageButton(container: HTMLElement, value: number) {
  return container.querySelector(`[data-slot="pagination-item"][aria-label="Page ${value}"]`) as HTMLButtonElement
}
...
it('支持非受控默认页，并在点击页码和导航按钮后同步 Reka 选择状态', async () => {
  ...
  await nextTick()
  expect((container.querySelector('[data-slot="pagination-item"][data-selected="true"]') as HTMLElement).getAttribute('aria-label')).toBe('Page 10')
...
it('按照 showEdges 与 siblingCount 生成页码范围和省略号', () => {
  ...
  const values = Array.from(page.container.querySelectorAll('[data-slot="pagination-item"]'))
...
it('在边界、零数据和根级禁用时禁用相应导航与页码操作', async () => {
  ...
  expect((boundaryContainer.querySelector('[data-slot="pagination-first"]') as HTMLButtonElement).disabled).toBe(true)
  expect((boundaryContainer.querySelector('[data-slot="pagination-prev"]') as HTMLButtonElement).disabled).toBe(true)

  const empty = render(createPagination({ paginationProps: { total: 0 } }))
  const emptyContainer = empty.container
  for (const slot of ['pagination-first', 'pagination-prev', 'pagination-next', 'pagination-last']) {
    expect((emptyContainer.querySelector(`[data-slot="${slot}"]`) as HTMLButtonElement).disabled).toBe(true)
  }

  const disabled = render(createPagination({ paginationProps: { defaultPage: 2, disabled: true } }))
...
it('支持调用方替换默认内容，以及 as 和 asChild 多态渲染', () => {
  const custom = render(createPagination({
  ...
  const PolymorphicFixture = defineComponent({
    setup: () => () => h(Pagination, { itemsPerPage: 10, total: 100 }, {
      default: () => h(PaginationList, {}, {
        default: () => [
          h(PaginationPrev, { asChild: true }, {
...
it('在亮暗主题下保留 Nova Token、紧凑尺寸与可见焦点样式', () => {
  ...
  expect(getComputedStyle(pagination).getPropertyValue('--background').trim()).toBe('oklch(0.145 0 0)')
})
