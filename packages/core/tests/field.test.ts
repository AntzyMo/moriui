...
it('渲染完整 Field 组合槽位，并保留语义、属性和调用方类名', () => {
  ...
  expect(page.container.querySelector('[data-slot="field-set"]')?.classList).toContain(
    'custom-set'
  )
  expect(page.container.querySelector('[data-slot="field-legend"]')?.tagName).toBe('LEGEND')
  expect(page.container.querySelector('[data-slot="field-legend"]')?.classList).toContain(
    'field__legend--label'
  )
  expect(page.container.querySelector('[data-slot="field-label"]')?.getAttribute('for')).toBe(
    'email'
  )
  expect(page.container.querySelector('[data-slot="field-content"]')?.classList).toContain(
    'field__content'
  )
  expect(page.container.querySelector('[data-slot="field-title"]')?.classList).toContain(
    'field__title'
  )
  expect(page.container.querySelector('[data-slot="field-separator"]')?.getAttribute('role')).toBe(
    'separator'
  )
  expect(page.container.querySelector('[data-slot="field-separator-content"]')?.textContent).toBe(
    '或使用其他邮箱'
  )
  expect(page.container.querySelector('[data-slot="field-error"]')?.getAttribute('role')).toBe(
    'alert'
  )
              ...
              { orientation: 'responsive' },
              {
                default: () => [
                  h(
                    FieldContent,
                    {},
                    { default: () => h(FieldTitle, {}, { default: () => '名称' }) }
                  ),
...
it('fieldError 优先渲染默认插槽，并过滤、去重和组织错误消息', () => {
  ...
  expect(slots.container.querySelector('[data-slot="field-error"]')?.textContent).toBe(
    '自定义错误'
  )
  ...
  expect(single.container.querySelector('[data-slot="field-error"]')?.textContent).toBe(
    '邮箱地址无效'
  )
...
it('响应错误、禁用和 Reka 选中状态，并继承亮暗主题 Token', () => {
                ...
                { orientation: 'horizontal' },
                {
                  default: () => [
                    ...
                    h(
                      FieldContent,
                      {},
                      { default: () => h(FieldTitle, {}, { default: () => '启用通知' }) }
                    )
                  ]
                }
              )
  ...
  const choiceLabel = page.container.querySelector(
    'label:has([data-slot="checkbox"])'
  ) as HTMLElement

  expect(getComputedStyle(invalidField).color).toBe(
    getComputedStyle(host).getPropertyValue('--destructive').trim()
  )
  expect(getComputedStyle(disabledLabel).opacity).toBe(
    getComputedStyle(host).getPropertyValue('--disabled-opacity').trim()
  )
  expect(page.container.querySelector('[data-slot="checkbox"]')?.getAttribute('data-state')).toBe(
    'checked'
  )
  expect(getComputedStyle(choiceLabel).borderColor).toBe(
