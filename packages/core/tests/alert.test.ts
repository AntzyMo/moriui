...
it.each(['default', 'destructive'] as const)('应用 %s 变体类', variant => {
  ...
  expect(page.container.querySelector('[data-slot="alert"]')?.classList).toContain(`alert--${variant}`)
})
...
it('允许 Alert 与描述槽位直接使用 Tailwind 类覆盖颜色', () => {
  const Fixture = defineComponent({
    setup: () => () => h(Alert, { class: 'bg-blue-50 text-blue-900' }, {
      default: () => h(AlertDescription, { class: 'text-blue-700' }, { default: () => '自定义颜色' })
    })
  })
  const page = render(Fixture)
  const alert = page.container.querySelector('[data-slot="alert"]') as HTMLElement
  const description = page.container.querySelector('[data-slot="alert-description"]') as HTMLElement
...
it('默认与破坏性变体消费语义 Token，并随主题切换 Card 表面', () => {
  ...
  const destructiveAlert = destructivePage.container.querySelector('[data-slot="alert"]') as HTMLElement
