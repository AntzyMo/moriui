...
function createFixture(options: { controlled?: boolean, size?: 'default' | 'sm' } = {}) {
  ...
  const Fixture = defineComponent({
    setup: () => () => h(AlertDialog, options.controlled
      ? { 'open': open.value, 'onUpdate:open': onUpdateOpen }
      : {}, {
      default: () => [
        h(AlertDialogTrigger, { class: 'custom-trigger' }, { default: () => '打开确认框' }),
        h(AlertDialogContent, { class: 'custom-content', size: options.size }, {
          default: () => [
            h(AlertDialogHeader, {}, {
              default: () => [
            ...
            h(AlertDialogFooter, {}, {
              default: () => [
                h(AlertDialogCancel, { asChild: true }, {
                  default: () => h(Button, { variant: 'outline' }, { default: () => '取消' })
                }),
                h(AlertDialogAction, { asChild: true }, {
                  default: () => h(Button, { variant: 'destructive' }, { default: () => '删除' })
                })
              ]
            })
          ]
...
async function openDialog(Fixture: ReturnType<typeof createFixture>['Fixture']) {
  ...
  const trigger = page.container.querySelector('[data-slot="alert-dialog-trigger"]') as HTMLButtonElement
  trigger.click()
  await nextTick()
  await nextTick()
  return { page, trigger }
...
it('渲染稳定槽位、Nova 类名、尺寸和按钮变体', async () => {
  ...
  const content = document.body.querySelector('[data-slot="alert-dialog-content"][data-state="open"]') as HTMLElement
  ...
  expect(document.body.querySelector('[data-slot="alert-dialog-overlay"]')?.classList).toContain('alert-dialog__overlay')
  expect(document.body.querySelector('[data-slot="alert-dialog-header"]')?.classList).toContain('alert-dialog__header')
  expect(document.body.querySelector('[data-slot="alert-dialog-media"]')?.classList).toContain('alert-dialog__media')
  expect(document.body.querySelector('[data-slot="alert-dialog-footer"]')?.classList).toContain('alert-dialog__footer')
  expect(document.body.querySelector('[data-slot="alert-dialog-action"]')?.classList).toContain('button--destructive')
  expect(document.body.querySelector('[data-slot="alert-dialog-cancel"]')?.classList).toContain('button--outline')
})
...
it('通过受控 open 模型同步开闭状态，并由操作按钮关闭', async () => {
  ...
  const action = document.body.querySelector('[data-slot="alert-dialog-content"][data-state="open"] [data-slot="alert-dialog-action"]') as HTMLButtonElement
  action.click()
  await nextTick()
...
it('提供 alertdialog 语义、标题描述关联与取消按钮初始焦点', async () => {
  ...
  const description = content.querySelector('[data-slot="alert-dialog-description"]') as HTMLElement
...
it('消费主题与短动效样式契约', async () => {
  ...
  await openDialog(Fixture)
  const content = document.body.querySelector('[data-slot="alert-dialog-content"][data-state="open"]') as HTMLElement
  const overlay = document.body.querySelector('[data-slot="alert-dialog-overlay"][data-state="open"]') as HTMLElement
  ...
  expect(getComputedStyle(content).getPropertyValue('--alert-dialog-bg').trim()).toBe('oklch(1 0 0)')
  document.documentElement.dataset.theme = 'dark'
  expect(getComputedStyle(content).getPropertyValue('--popover').trim()).toBe('oklch(0.205 0 0)')
  expect(getComputedStyle(content).getPropertyValue('--alert-dialog-bg').trim()).toBe('oklch(0.205 0 0)')
  delete document.documentElement.dataset.theme
})
