...
it('支持点击、受控 v-model 与完整默认作用域插槽', async () => {
  ...
  const radios = Array.from(
    page.container.querySelectorAll('[data-slot="radio-group-item"]')
  ) as HTMLButtonElement[]

  expect(radios[1].getAttribute('aria-checked')).toBe('true')
  expect(page.container.querySelector('[data-testid="custom-indicator-weekly"]')?.textContent).toBe(
    'true-false-false'
  )
  ...
  expect(page.container.querySelector('[data-testid="custom-indicator-daily"]')?.textContent).toBe(
    'true-false-false'
  )
})
...
it('支持组级禁用、横向排列、焦点与 Nova 无效状态', async () => {
  ...
  const page = render(Fixture)
  const [disabledGroup, invalidGroup] = Array.from(
    page.container.querySelectorAll('[data-slot="radio-group"]')
  ) as HTMLElement[]
  const disabledRadios = Array.from(
    disabledGroup.querySelectorAll('[data-slot="radio-group-item"]')
  ) as HTMLButtonElement[]
