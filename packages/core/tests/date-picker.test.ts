...
it('单日期选择器支持分段输入、弹层日历、键盘选择和默认选后关闭', async () => {
  ...
  await nextTick()
  expect((page.container.querySelector('[data-testid="value"]') as HTMLElement).textContent).toBe(
    '2024-04-10'
  )
  expect(
    (document.querySelector('[data-slot="date-picker-content"]') as HTMLElement).dataset.state
  ).toBe('closed')
})
...
it('日期范围选择器在起止日期完整后关闭，并呈现范围状态', async () => {
  ...
  expect((page.container.querySelector('[data-testid="range"]') as HTMLElement).textContent).toBe(
    '2024-04-10|2024-04-15'
  )
  expect(
    (document.querySelector('[data-slot="date-range-picker-content"]') as HTMLElement).dataset
      .state
  ).toBe('closed')
})
